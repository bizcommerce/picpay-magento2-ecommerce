...
namespace PicPay\Checkout\Model\Total\Quote;
...
class Interest extends AbstractTotal
{
    ...
    protected function getInstallments()
    {
        ...
        /** @var Quote $quote */
        //Prepared for PicPay transparent
        $quoteId = $this->checkoutSession->getQuoteId();
        if ($quoteId) {
            $quote = $this->quoteRepository->get($quoteId);
            if ($quote->getPayment()->getMethod() == ConfigProvider::CODE) {
                $installments = (int) $this->checkoutSession->getData('picpay_installments');
            }
        }
    ...
    protected function getInterestAmount(): float
    {
        ...
        if ($installments > 1) {
            ...
            if ($quoteId) {
                /** @var Quote $quote */
                $quote = $this->quoteRepository->get($quoteId);
                $grandTotal = $quote->getGrandTotal() - $quote->getPicpayInterestAmount();
    ...
    {
        try {
            $allInstallments = $this->helperInstallments->getAllInstallments($grandTotal);
    ...
    public function collect(
        ...
        $quote->setPicpayInterestAmount($interest);
        $quote->setBasePicpayInterestAmount($interest);

        $total->setPicpayInterestDescription($this->getCode());
        $total->setPicpayInterestAmount($interest);
        $total->setBasePicpayInterestAmount($interest);
    ...
    {
        $result = null;
        $amount = $total->getPicpayInterestAmount();
